...
if strcmp(par.plotSelect,'1')
        ...
        plotOut = plotrbfUR5_4(par, 'critic', par.plotopt); title(['\bf{CRITIC}  Iteration: ' int2str(trial_count)]); colorbar;
        ...
        plotOut = plotrbfUR5_4(par, 'actor', par.plotopt); title('\bf{ACTOR}');  colorbar;
        hold on; plot(wTable(3,:), wdTable(3,:), 'r.');
        plot(linspace(par.xmin, par.xmax, size(wrTraj,2)), zeros(1, size(wrTraj,2)));
...
%         positionVector3 = [0.05, 0.305, 0.43, 0.16];
        ...
        deltaZ = wrTraj(3,2:end)-wrTraj(3,2:end);
%         wnomplotTable(3,:) = wnomTable(3,:)+deltaZ;
        plot(time(2:end), 1000*wrTraj(3,2:end), 'red'); hold on;
%         plot(time,1000*wnomplotTable(3,:), 'green');   
        if (trial_count>0);            
        plot(time(2:end), 1000*wrTable(3,2:end), 'black');
        plot(time(2:end), 1000*wTable(3,2:end), 'blue');   
        end
...
%         xlabel('trials');
        ...
        plot(time(1:end-1), 1000*dref, time(1:end-1), 1000*du(1:end), 'r');
        xlabel('time (seconds)'); ylabel('additive modifier (mm)'); xlim([0 par.t_end]); title('Additive modifier (b) & exploration (r)');       
        end
