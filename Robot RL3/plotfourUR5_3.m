function y = plotfourUR5_3(par, opt, opt2)
%plotfourUR5_3 plot the approximated function using fourier basis
% 
% Y = plotfourUR5_3(PAR, OPT, OPT2) plot the approximated function of the
...
x1      = linspace(par.cBF.r(2,1), par.cBF.r(2,2), N);
x2      = linspace(par.cBF.r(1,1), par.cBF.r(1,2), N);

if strcmp(opt, 'actor1')
    for i = 1:length(x1)
        for j = 1:length(x2)
            [PhiA1, ~]	= fourUR5_3([x1(i); x2(j)], par, 'actor1');
            V(i,j)    	= phi1'*PhiA1;
        end
    end
    if strcmp(opt2, '2d')
...
elseif strcmp(opt, 'actor2')
    for i = 1:length(x1)
        for j = 1:length(x2)
            [PhiA2, ~]	= fourUR5_3([x1(i); x2(j)], par, 'actor2');
            V(i,j)    	= phi2'*PhiA2;
        end
    end
    if strcmp(opt2, '2d')
...
elseif strcmp(opt, 'critic')
    for i = 1:length(x1)
        for j = 1:length(x2)
            [PhiC, ~]	= fourUR5_3([x1(i); x2(j)], par, 'critic');
            V(i,j)    	= theta'*PhiC;
        end
    end
    if strcmp(opt2, '2d')
