%robotRLTest4 is a script to test the RL-based additive reference modifier to 
...
for trial_count = 0:par.Ntrial
    ...
    q0       	= arm.getJointsPositions(); % initial joint position
    r0      	= w0(4:end);                % initial orientation
    wrTraj2     = zeros(6,N);   % reference trajectory, will be populated in the iteration
    offset    	= [-0.15 0 0 0 0 0]';
    wf          = w0 + offset;
    dOffset    	= offset/N;

    for i = 1:N
        wrTraj2(:,i)	= w0 + (i-1)*dOffset; % populate reference trajectory
    end
        ...
        for i=1:N-1
            ...
            if ~mod(i,par.expSteps)  	% explore only once every defined time steps  
                if ~sat_flag(i-1) && trial_count>1
                    if uad(trial_count,i) > 0 % too high, bring down
...
%             qrTraj{trial_count+1}(4:6,i+1)      = qTable(4:6,1);
            ...
            [r(i+1), r1(i+1), r2(i+1)] = costUR5_4([wTable(3,i) wdTable(3,i)], [wrTraj2(3,i) 0], par); 	% calculate the immediate cost 
