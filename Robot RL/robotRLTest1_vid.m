...
%     params.meanRand =  -0.1*mean(diff/max(diff))*params.uSat;
    ...
    for i=1:N-1
        ...
        w                   = wTable(:,k);  % get current joint position
        ...
        tic
        arm.setJointsSpeed(qdotRefu(:,k),params.acc,6*SAMPLING_TIME);
        ...
        toolVel(:,k)        = arm.getToolSpeeds();               % tool velocity
...
%         end
    ...
    if mod(counter,params.plotSteps) == 0
        ...
        xlabel('$z  \hspace{1mm}$ [mm]','Interpreter','Latex'); ylabel('$V(z)$ \hspace{1mm} [-]','Interpreter','Latex'); %colorbar 
        subplot(322); 
        plotOut = plotrbfUR5_1(params, 'actor'); title('\bf{ACTOR}'); 
        xlabel('$z  \hspace{1mm}$ [mm]','Interpreter','Latex'); ylabel('$\pi(z)$ \hspace{1mm} [-]','Interpreter','Latex'); %colorbar 
        subplot(323);
